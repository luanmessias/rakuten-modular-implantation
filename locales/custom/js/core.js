    ...
    if (jQuery('.list-products .hproduct').length) {
        jQuery('.list-products .hproduct').each(function () {
            if (!jQuery(this).find('.offers > .percent-a-vista').length) {
                jQuery('.offers .price-to', this).after('<span class="price sale percent-a-vista" style="display:block; width:100%;"></span>')
            }
            ...
            if (!jQuery(this).find('.offers > .price-from').length && pageW <= 979) {
                jQuery('.priceCurrency', this).after('<div class="regular price-from"></div>');
            }
        });
        jQuery('.list-products .hproduct .photo').matchHeight({
            byRow: true
        });
